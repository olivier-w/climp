...
type spectral_huffman_code struct {
	Values [4]int
	Count  uint8
}

type spectral_data struct {
	Hcod           []spectral_huffman_code
...
func (adts *ADTS) spectral_data(info *ics_info, sec_data *section_data) (*spectral_data, error) {
	...
	for g := uint8(0); g < info.num_window_groups; g++ {
		for i := uint8(0); i < sec_data.num_sec[g]; i++ {
			...
			if end > start {
				totalCodewords += int((end - start) / inc)
			}
		}
	}
	data.Hcod = make([]spectral_huffman_code, totalCodewords)
	codewordIndex := 0
			...
			default:
				...
				for k := start; k < end; k += inc {
					if sec_data.Sect_cb[g][i] != 0 {
						...
						if codewordIndex >= len(data.Hcod) {
							return data, fmt.Errorf("Error: Spectral Data codeword count mismatch")
						}
						val := &data.Hcod[codewordIndex]
						err := hcod(adts.reader, sec_data.Sect_cb[g][i], val)
						if err != nil {
							return data, err
						}
						codewordIndex++
